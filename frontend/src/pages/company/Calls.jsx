...
export default function CompanyCalls() {
  const { data, isLoading, error } = useFetch('manager-calls', '/calls?limit=50');
  ...
  return (
    <div className="p-6 md:p-8">
      <div className="mb-6">
        ...
        <p className="mt-1 text-sm text-gray-500">View all employee calls across the company.</p>
      </div>
            ...
            <tbody className="divide-y divide-gray-200">
              {calls.map(c => (
                <tr key={c.id} className="hover:bg-gray-50">
                  <td className="px-6 py-4 text-sm font-medium text-gray-900">{c.user_name || '—'}</td>
                  <td className="px-6 py-4 text-sm text-gray-600">{c.lead_name}</td>
